    ...
    <>
      <aside
        className={`fixed  top-0 ${
          aside ? "left-0" : "-left-[1000px]"
        } transition-all duration-700 h-screen w-full bg-emerald-900  flex flex-col lg:hidden items-center gap-10 justify-center p-5 z-10`}
      >
        <button></button>
        <Link to={"/dashboard"} onClick={() => setAside(false)}>
      ...
      <main className="flex">
        <aside className="w-1/5 bg-emerald-800 h-screen hidden lg:flex flex-col gap-10 p-5 ">
          ...
          <Link
            to={"/dashboard"}
            className="relative flex items-center gap-3 group text-start text-emerald-100 capitalize text-xl overflow-hidden"
          >
            <HomeIcon className="h-5 w-5 text-emerald-500" aria-hidden="true" />
            Inicio
          ...
          <button
            ...
            className="relative flex items-center gap-3 group text-start text-emerald-100 capitalize text-xl overflow-hidden"
          >
            <ArrowLeftOnRectangleIcon  className="h-5 w-5 text-emerald-500"
            aria-hidden="true"/>
            Cerrar sesion
            <span className="absolute inset-x-0 w-0 bottom-0 h-0.5 bg-white group-hover:w-full transition-all"></span>
          </button>
